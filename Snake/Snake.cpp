...
int main( )
{
	...
	while ( true )
	{
		...
		while ( game_controller.get_player_state( ) == e_player_state::ALIVE )
		{
			...
			game_controller.move_snake( );
			game_controller.update_states( );

			game_interface.clear_area( );
			game_interface.draw_apple( game_controller.get_apple( ) );
			game_interface.draw_snake( game_controller.get_snake_head( ), game_controller.get_snake_tail( ) );
		}
	...
	while ( true )
	{
		if ( game_controller.get_player_state( ) == e_player_state::ALIVE )
		{
			if ( GetKeyState( VK_UP ) & 0x8000 )
				game_controller.change_move_direction( e_move_direction::UP );

			if ( GetKeyState( VK_RIGHT ) & 0x8000 )
				game_controller.change_move_direction( e_move_direction::RIGHT );

			if ( GetKeyState( VK_DOWN ) & 0x8000 )
				game_controller.change_move_direction( e_move_direction::DOWN );

			if ( GetKeyState( VK_LEFT ) & 0x8000 )
				game_controller.change_move_direction( e_move_direction::LEFT );
		}
		else
		{
